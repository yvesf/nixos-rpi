...
let
  ...
  configOptions' = recursiveUpdate configOptions
    (if !cfg.enableCollectd then { collectd = [{enabled = false; typesdb = "";}]; } else {});
  ...
  configFile = pkgs.runCommand "config.toml" {
    ...
    remarshal -if json -of toml \
       < ${pkgs.buildPackages.writeText "config.json" (builtins.toJSON configOptions')} \
       > $out
  '';
in
{
  ...
  options = {
    ...
    services.influxdb = {
      ...
      enableCollectd = mkOption {
        default = true;
        description = "set to false to disable dependency on collectd";
        type = types.bool;
      };
